...
namespace XCharts.Runtime
{
    ...
    internal sealed class SimplifiedCandlestickHandler : SerieHandler<SimplifiedCandlestick>
    {
        ...
        private void DrawCandlestickSerie(VertexHelper vh, SimplifiedCandlestick serie)
        {
            ...
            if (!chart.TryGetChartComponent(out grid, xAxis.gridIndex)) return;
            ...
            for (var i = serie.minShow; i < maxCount; i++)
            {
                ...
                var lowPos = new Vector3(center.x,
                    zeroY + (float)((lowest - minCut) / valueTotal * grid.context.height));
                var heighPos = new Vector3(center.x,
                    zeroY + (float)((heighest - minCut) / valueTotal * grid.context.height));
