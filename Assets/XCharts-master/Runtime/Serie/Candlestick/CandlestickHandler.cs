...
namespace XCharts.Runtime
{
    ...
    internal sealed class CandlestickHandler : SerieHandler<Candlestick>
    {
        ...
        private void DrawCandlestickSerie(VertexHelper vh, Candlestick serie)
        {
            ...
            XAxis xAxis;
            YAxis yAxis;
            if (!chart.TryGetChartComponent(out xAxis, serie.xAxisIndex)) return;
            if (!chart.TryGetChartComponent(out yAxis, serie.yAxisIndex)) return;
            if (!chart.TryGetChartComponent(out m_SerieGrid, xAxis.gridIndex)) return;
            var theme = chart.theme;
            var dataZoom = chart.GetDataZoomOfAxis(xAxis);
            var showData = serie.GetDataList(dataZoom);
            var categoryWidth = AxisHelper.GetDataWidth(xAxis, m_SerieGrid.context.width, showData.Count, dataZoom);
            ...
            for (var i = serie.minShow; i < maxCount; i++)
            {
                ...
                var open = serieData.GetCurrData(startDataIndex, dataAddDuration, dataChangeDuration, yAxis.inverse,
                    yMinValue, yMaxValue, unscaledTime);
                var close = serieData.GetCurrData(startDataIndex + 1, dataAddDuration, dataChangeDuration,
                    yAxis.inverse, yMinValue, yMaxValue, unscaledTime);
                var lowest = serieData.GetCurrData(startDataIndex + 2, dataAddDuration, dataChangeDuration,
                    yAxis.inverse, yMinValue, yMaxValue, unscaledTime);
                var heighest = serieData.GetCurrData(startDataIndex + 3, dataAddDuration, dataChangeDuration,
                    yAxis.inverse, yMinValue, yMaxValue, unscaledTime);
                ...
                var pX = xAxis.IsCategory()
                    ? m_SerieGrid.context.x + i * categoryWidth
                    : AxisHelper.GetAxisValuePosition(m_SerieGrid, xAxis, categoryWidth, serieData.GetData(0));
                var zeroY = m_SerieGrid.context.y + yAxis.context.offset;
                ...
                var lowPos = new Vector3(center.x,
                    zeroY + (float)((lowest - minCut) / valueTotal * m_SerieGrid.context.height));
                var heighPos = new Vector3(center.x,
                    zeroY + (float)((heighest - minCut) / valueTotal * m_SerieGrid.context.height));
                ...
                else
                {
                    ...
                    {
                        UGL.DrawLine(vh, openCenterPos, closeCenterPos, Mathf.Max(borderWidth, barWidth / 2),
                            borderColor);
                    }
            ...
            if (!serie.animation.IsFinish()) serie.animation.CheckProgress();
