...
namespace XCharts.Runtime
{
    ...
    public class LabelStyle : ChildComponent, ISerieComponent, ISerieDataComponent
    {
        ...
        [SerializeField] [Since("v3.15.0")] protected float m_FixedX;
        [SerializeField] [Since("v3.15.0")] protected float m_FixedY;
        ...
        public float fixedX
        {
            ...
            set
            {
                if (PropertyUtil.SetStruct(ref m_FixedX, value)) SetComponentDirty();
            }
        }
        ...
        public float fixedY
        {
            ...
            set
            {
                if (PropertyUtil.SetStruct(ref m_FixedY, value)) SetComponentDirty();
            }
        }
        ...
        {
            if (ChartHelper.IsClearColor(textStyle.color)) return IsInside() ? Color.black : defaultColor;
        ...
        public virtual string GetFormatterContent(int labelIndex, int totalIndex, string category)
        {
            ...
            if (string.IsNullOrEmpty(m_Formatter)) return GetFormatterFunctionContent(labelIndex, category, category);
        ...
        public virtual string GetFormatterContent(int labelIndex, int totalIndex, double value, double minValue,
            double maxValue, bool isLog = false)
        {
            ...
            {
                if (isLog)
                    return GetFormatterFunctionContent(labelIndex, value,
                        ChartCached.NumberToStr(value, newNumericFormatter));
                if (minValue >= -1 && minValue <= 1 && maxValue >= -1 && maxValue <= 1)
                {
                    ...
                    return GetFormatterFunctionContent(labelIndex, value,
                        ChartCached.FloatToStr(value, newNumericFormatter, acc));
                }
        ...
        public string GetFormatterDateTime(int labelIndex, int totalIndex, double value, double minValue,
            double maxValue, bool local)
        {
            var timestamp = value;
        ...
        {
            return m_FormatterFunction == null
                ? currentContent
