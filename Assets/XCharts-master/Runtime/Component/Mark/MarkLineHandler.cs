    ...
    internal sealed class MarkLineHandler : MainComponentHandler<MarkLine>
    {
        ...
        private void InitMarkLineLabel(Serie serie, MarkLineData data, Color serieColor)
        {
            ...
            data.refreshComponent = delegate
            {
                ...
                var label = ChartHelper.AddChartLabel(textName, m_MarkLineLabelRoot.transform, data.label,
                    chart.theme.axis,
                    content, Color.clear);
                var pos = MarkLineHelper.GetLabelPosition(data);
        ...
        private void ResetTempMarkLineGroupData(MarkLine markLine)
        {
            ...
            for (var i = 0; i < markLine.data.Count; i++)
            {
                ...
                data.index = i;
                if (data.group == 0) continue;
                if (!m_TempGroupData.ContainsKey(data.group)) m_TempGroupData[data.group] = new List<MarkLineData>();
            ...
            GridCoord grid, Color32 serieColor, Vector3 sp, Vector3 ep)
        {
            ...
            if (sp != Vector3.zero || ep != Vector3.zero)
            {
                ...
                ChartDrawer.DrawLineStyle(vh, data.lineStyle, sp, ep, lineWidth, LineStyle.Type.Dashed, lineColor,
                    lineColor);
        ...
        private void DrawMarkLineSymbol(VertexHelper vh, SymbolStyle symbol, Serie serie, GridCoord grid,
            ThemeStyle theme,
        ...
        private Vector3 GetSinglePos(Axis xAxis, Axis yAxis, GridCoord grid, Serie serie, DataZoom dataZoom,
            MarkLineData data,
            int serieDataCount)
        {
            switch (data.type)
            {
                case MarkLineType.Min:
                    var serieData = SerieHelper.GetMinSerieData(serie, data.dimension);
                ...
                case MarkLineType.Max:
                    serieData = SerieHelper.GetMaxSerieData(serie, data.dimension);
                    data.runtimeValue = serieData.GetData(data.dimension);
