...
namespace XCharts.Runtime
{
    ...
    internal sealed class TooltipHandler : MainComponentHandler<Tooltip>
    {
        ...
        public override void OnPointerExit(PointerEventData eventData)
        {
            ...
            if (chart.isTriggerOnClick) component.context.xAxisClickIndex = -1;
        }
        ...
        private ChartLabel GetIndicatorLabel(Axis axis)
        {
            ...
            if (m_IndicatorLabels.ContainsKey(key)) return m_IndicatorLabels[key];
        ...
        private void UpdateTooltip(Tooltip tooltip)
        {
            ...
            {
                if (tooltip.context.type == Tooltip.Type.Cross && m_PointerContainer != null &&
                    m_PointerContainer.IsPointerEnter())
                {
                    m_ShowTooltip = true;
        ...
        private void SetTooltipIndicatorLabel(Tooltip tooltip, Axis axis, ChartLabel label)
        {
            ...
            {
                label.SetText(axis.indicatorLabel.GetFormatterDateTime(0, 0, axis.context.pointerValue,
                    axis.context.minValue, axis.context.maxValue, !chart.useUtc));
            }
            else
            {
                label.SetText(axis.indicatorLabel.GetFormatterContent(0, 0, axis.context.pointerValue,
                    axis.context.minValue, axis.context.maxValue, axis.IsLog()));
            }
                        ...
                        foreach (var serie in chart.series)
                                ...
                                !serie.placeHolder)
                            {
                                ...
                                if (container is GridCoord)
                                {
                                    ...
                                    inchart = UpdateAxisPointerDataIndex(serie, xAxis, yAxis, container as GridCoord,
                                        isTriggerAxis);
                                }
                                ...
                                {
                                    list.Add(serie);
                                    if (!isTriggerAxis) chart.RefreshTopPainter();
                                }
                            }
        ...
        private bool UpdateAxisPointerDataIndex(Serie serie, XAxis xAxis, YAxis yAxis, GridCoord grid,
            bool isTriggerAxis)
        {
            ...
            else if (xAxis.IsCategory())
            {
                if (isTriggerAxis)
                {
                    ...
                    if (component.context.xAxisClickIndex != index)
                    {
                        ...
                        if (component.onClickIndex != null) component.onClickIndex(index);
                    }
        ...
        private void GetSerieDataIndexByAxis(Serie serie, Axis axis, GridCoord grid, int dimension = 0)
        {
            ...
            for (var i = 0; i < dataCount; i++)
            {
                ...
                {
                    nextValue = data[i + 1].GetData(dimension);
                    if (axisValue > currValue - (currValue - lastValue) / 2 &&
                        axisValue <= currValue + (nextValue - currValue) / 2)
                    {
                        serie.context.pointerAxisDataIndexs.Add(serieData.index);
                        break;
                    }
        ...
        private void GetSerieDataIndexByItem(Serie serie, Axis axis, GridCoord grid, int dimension = 0)
        {
            if (serie.context.pointerItemDataIndex >= 0)
                axis.context.axisTooltipValue =
                    serie.GetSerieData(serie.context.pointerItemDataIndex).GetData(dimension);
        ...
        private bool SetSerieTooltip(Tooltip tooltip, Serie serie)
        {
            ...
            tooltip.context.trigger =
                tooltip.trigger == Tooltip.Trigger.Auto ? serie.context.tooltipTrigger : tooltip.trigger;
        ...
        private bool SetSerieTooltip(Tooltip tooltip, List<Serie> series)
        {
            ...
            for (var i = 0; i < series.Count; i++)
            {
                ...
                if (timestamp >= 0)
                {
                    ...
                    if (serieData != null)
                    {
                        ...
                        {
                            tooltip.context.data.title =
                                DateTimeUtil.GetDefaultDateTimeString(value, axisRange, !chart.useUtc);
                        }
                        else
                        {
                            ...
                            {
                                tooltip.context.data.title =
                                    dateTime.ToString(tooltip.titleLabelStyle.numericFormatter);
                            }
                            catch
                            {
                                tooltip.context.data.title =
            ...
            if (triggerSerieCount <= 0) return false;
            TooltipHelper.ResetTooltipParamsByItemFormatter(tooltip, chart);
        ...
        private bool GetAxisCategory(int gridIndex, ref int dataIndex, ref string category, ref int timestamp,
            ref double axisRange)
        {
            foreach (var component in chart.components)
                ...
                if (xAxis.gridIndex == grid.index)
                {
                    ...
                    switch (tooltip.context.type)
                    {
                        ...
                        case Tooltip.Type.Line:
                            ...
                            pX += xAxis.IsCategory()
                                ? (float)(xAxis.context.pointerValue * splitWidth +
        ...
        private void DrawPolarIndicator(VertexHelper vh, Tooltip tooltip, PolarCoord m_Polar)
        {
            ...
            switch (tooltip.context.type)
            {
                ...
                case Tooltip.Type.Line:
                    ...
                    break;
                case Tooltip.Type.Shadow:
                    UGL.DrawSector(vh, cenPos, radius, lineColor, tooltipAngle - 2, tooltipAngle + 2,
