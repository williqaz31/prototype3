...
namespace XUGL
{
    public static class UGLHelper
    {
        ...
        public static List<Vector3> GetBezierList(Vector3 sp, Vector3 ep, int segment, Vector3 cp)
        {
            ...
            for (var i = 0; i < segment; i++) list.Add(GetBezier(i / (float)segment, sp, cp, ep));
            list.Add(ep);
            return list;
        }
        ...
        {
            posList.Clear();
            if (posList.Capacity < segment + 1) posList.Capacity = segment + 1;
            for (var i = 0; i < segment; i++) posList.Add(GetBezier2(i / (float)segment, sp, cp, cp2, ep));
            posList.Add(ep);
        }
        ...
        public static bool GetIntersection(Vector3 p1, Vector3 p2, Vector3 p3, Vector3 p4, ref Vector3 intersection)
        {
            ...
            var u = (dx3 * dy2 - dy3 * dx2) / d;
            if (u < 0 || u > 1) return false;

            var v = (dx3 * dy1 - dy3 * dx1) / d;
        ...
        /// <param name="intersection">相交点。当不相交时为初始值</param>
        /// <returns>相交则返回 true, 否则返回 false</returns>
        public static bool GetIntersection(Vector3 p1, Vector3 p2, Vector3 p3, Vector3 p4,
            ref List<Vector3> intersection)
        {
            ...
            ref bool bitp, ref bool bibp, int debugIndex = 0)
        {
            ...
            var crossMagnitude = Vector3.Cross(dir1, dir2).sqrMagnitude;
            ...
            bitp = GetIntersection(ltp, ltp + ldist, ntp, ntp + rdist, ref itp);
            bibp = GetIntersection(lbp, lbp + ldist, nbp, nbp + rdist, ref ibp);
            if (bitp == bibp)
            {
                if (!bitp)
                {
                    ...
                    {
                        var ibp2 = Vector3.zero;
                        if (GetIntersection(lbp, lbp + ldist, ntp, nbp, ref ibp2))
                        {
                            bibp = true;
                            ibp = ibp2;
                            clp = cp - dir1v;
                        ...
                        else if (GetIntersection(ltp, ltp + ldist, nbp, ntp, ref ibp2))
                        {
                            bitp = true;
                            itp = ibp2;
                            clp = cp + dir1v;
