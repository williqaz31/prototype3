...
public class PopCounter : MonoBehaviour
{
...

    {
        try
        {
            ...
            if (simState.Colonie.Pop() == 0) GameOver?.Invoke();
            var x = simState.Jour.ToString();
    ...
    public void NouvelleSemaine(bool isWeekly)
    {
        ...
        if (isWeekly) simState.ResetCounters();
    }
    ...
    public string EndWeek()
    {
        ...
        var popEndWeek = simState.Colonie.Pop();

        var details = getInfo(popEndWeek, popStartWeek);
    ...
    {
        var popEndMonth = simState.Colonie.Pop();

        var details = getInfo(popEndMonth, popStartMonth);
    ...
    private string getInfo(int popEnd, int popStart)
    {
        ...
        if (simState.Famine)
        {
            ...
            else if (simState.DebutFamine != null) durer = (int)(simState.Jour - simState.DebutFamine);
        ...
        {
            famine =
                $"<color=\"green\">Félicitation Il n'y à pas eu de famine\nScore de famine: {simState.Score}</color >";
        }


        var morts_naturels = popStart - popEnd - simState.Naissance - simState.MortsAffame;
        var morts = popStart - popEnd;
        if (morts < 0) morts = 0;
        if (morts_naturels < 0) morts_naturels = 0;
        return
