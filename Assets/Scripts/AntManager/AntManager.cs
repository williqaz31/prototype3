...
public class AntManager : MonoBehaviour
{
    ...
    void SyncAntCount()
    {
        ...
        while (ants.Count < target) SpawnSingleAnt();
        while (ants.Count > target) RemoveAnt();
    }
    ...
    List<Vector2Int> FindPath(Vector2Int start, Vector2Int goal)
    {
        ...
        var step = goal;
        while (step != start) { path.Add(step); step = cameFrom[step]; }
        path.Reverse();
        return path;
    }
    ...
    {
        var mapData = mapLoader.GetMapData();
        if (p.x < 0 || p.y < 0 || p.y >= mapLoader.Rows || p.x >= mapLoader.Cols) return false;
    ...
    void AssignJobs()
    {
        char[,] mapData = mapLoader.GetMapData();
        foreach (var job in mapLoader.jobs)
        {
            if (job.taken) continue;
            if (job.target.x < 0 || job.target.y < 0 || job.target.x >= mapLoader.Cols || job.target.y >= mapLoader.Rows)
            {
                job.taken = true;
                continue;
            }

            if (mapData[job.target.y, job.target.x] == '5')
            {
                job.taken = true;
                continue;
            }
    ...
    {
        foreach (int id in antIds)
            if (!antTargets.ContainsKey(id)) return id;
        return -1;
    }
    ...
    {
        foreach (int id in antIds)
            if (!antPaths.ContainsKey(id) && antStates[id] == AntState.Idle)
            {
                AssignJobs();
                break;
            }
    ...
    public void UpdateAnts()
    {
        foreach (int id in antIds)
        {
            ...
            if (antMoveTimers[id] < moveInterval) continue;
            antMoveTimers[id] = 0f;
    ...
    void OnAntArrived(int antIndex, Vector2Int pos)
    {
        ...
        if (state == AntState.GoingToDig)
        {
            ...
            if (exit == null) { antStates[antIndex] = AntState.Idle; antTargets.Remove(antIndex); return; }
            var path = FindPath(ants[antIndex], exit.Value);
            if (path == null) { antStates[antIndex] = AntState.Idle; antTargets.Remove(antIndex); return; }
    ...
    {
        foreach (int id in antIds) ants[id] += delta;
        var keys = new List<int>(antTargets.Keys);
        foreach (var k in keys) antTargets[k] += delta;
        foreach (var path in antPaths.Values)
        {
            var newQueue = new Queue<Vector2Int>();
            foreach (var p in path) newQueue.Enqueue(p + delta);
            path.Clear();
            foreach (var p in newQueue) path.Enqueue(p);
        }
    }
}